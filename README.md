...
- Frozen tiles
- 1-3 tile swaps
- Ability to prioritize gems (configurable)

But some things are missing:

- Doesn't recommend shuffling (use your brain to figure out when to shuffle)
- No "gem management" (use your brain to figure out when to enable gem prioritizing)
...
Additional options can be passed as command-line options.
...
- `-B`/`--benchmark` - run benchmark instead of solver (every other argument is ignored) (it's a flag, defaults to `false` if not specified)

For example, if you have two swaps, want to somewhat prioritize gems (+3 per gem), want to pretty-print the moves and want to specify board in command line, you can run `./target/release/spellcast-solver -p -g 3 -s 2 -b "board string here"`
...
As was mentioned before, you can run benchmark if you specify `-B`/`--benchmark` command line argument (e.g. `./target/release/spellcast-solver -B`). Like in original, it generates 1000 random boards and solves them, giving some results.

Here are benchmark results from `stable-x86_64-unknown-linux-gnu` with `RUSTFLAGS="-C target-cpu=native"` on my mediocre computer with 11th gen Intel Core i5. These are approximate average results you should expect from this program.
